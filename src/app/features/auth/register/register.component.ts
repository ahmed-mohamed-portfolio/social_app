...
export class RegisterComponent implements OnInit {
  ...
  // custom validation if password != confirmPassword
  matchValid(vGroup: AbstractControl) {
    if (vGroup.get('password')?.value === vGroup.get('confirmPassword')?.value) {
      return null;
    } else {
      vGroup.get('confirmPassword')?.setErrors({ matchpassword: true });
    ...
    this.subscribe = this.path.signUp(this.registerForm.value).subscribe({
      next: (res) => {
        ...
        if (res.message == 'success') {
          ...
          this.toastrService.success('please verify your email');
